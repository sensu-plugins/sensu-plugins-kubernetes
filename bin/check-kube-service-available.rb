...
class AllServicesUp< Sensu::Plugin::Check::CLI
    ...
    def run
      ...
      failed_services = Array.new
      s = client.get_services
      s.each do |a|
        if services.include?(a.metadata.name)
            #Build the selector key so we can fetch the corresponding pod
            selector_key = Array.new
            ...
            pod = nil
            begin
              pod = client.get_pods(label_selector: "#{selector_key.join(",")}")
            rescue
              failed_services << "#{a.metadata.name}"
            end
            #Make sure our pod is running
            if not pod.nil?
                pod.each do |p|
                  if not p.status.phase.include?('Running')
      ...
      if failed_services.empty? and services.empty?
